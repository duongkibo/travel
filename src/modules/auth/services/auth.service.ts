...
export class AuthService {
  ...
  validateUser = async (
    ...
    if (user) {
      ...
      throw new NotFoundException(UserCommonEnum.LOGIN_FAIL);
    }
  };
  ...
  saveAuthToken = async (userId: string, email: string, options?: JwtGenerateOption) => {
  ...
  refreshToken = async (refreshToken: string) => {
    try {
      ...
      const user = this.usersService.findOne(currentPayload.email);
      if (newToken) {
        return {
          user,
